...
\begin{Exercise}[title= Reading files] \label{Ex:Variables}
	...
	\Question{Print the third row of the table (excluding the headings).}
	\Question{Print the numerical data in the column Mass as a list of {\tt float} values, shown in grams (g).}
\end{Exercise}


\begin{Exercise}[title= Reading and writing files] \label{Ex:Variables}
	\Question{Write a program that edits the Mass of sample B to be 0.465 kg}
	\Question{Write a program that edit the table so that all Mass data is rounded to 2 significant figures (the nearest 50g)}
