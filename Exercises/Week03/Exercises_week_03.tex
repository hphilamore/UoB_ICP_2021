...
\begin{Exercise}[title=While Loops (Essential)]
	...
	\Question{Can you rewrite Q\ref{Q:whilestring} using a {\tt for} loop, an {\tt if} clause and {\tt enumerate()}? \\ \textbf{Hint:} research the {\tt enumerate} function. To get a feel for how this works, run the following code
          \begin{minted}{python}
            s = 'hello'
            enum = enumerate(s)
            print(list(enum))
...
\begin{Exercise}[title=More Loops (Advanced)]
  ...
  \Question{The value of $\pi$ can be approximated using the Leibniz formula:
    ...
    \Question{The Fibonacci sequence is a sequence of numbers where each number
      is the sum of the two preceeding ones. The first two
...
\begin{Exercise}[title=FizzBuzz Game (Advanced)]
