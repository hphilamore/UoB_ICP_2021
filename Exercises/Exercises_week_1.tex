...
\ExeText{Python can be used as a calculator. You can input operations, and store the results of operations as variables for use in additional calculations.}
	...
	\Question{Calculate the product of {\tt A} and {\tt B} and print the result.}
	\Question{Calculate $\frac{A+B}{3}$ (Hint: Python follows the same ordering of mathematical operations as any other calculator).
	\Question{Find the remainder when {\tt A} is divided by {\tt B} and print the result.}.
    \Question{Write a program to output that stores the radius of a sphere as a variable and outputs the volume of the sphere.}
    \Question{Create 3 variables for the length, width and height of a cuboid. Write a program to output the volume of the cuboid.}
    \end{Exercise}
...
% 		\vspace{0.5em}}
    ...
    \Question{Create two variables {\tt C} and  {\tt D}.}
	\Question{Assign the value {\tt `Hello '} to {\tt C} and  the value {\tt `World'} to {\tt D}. Use an arithmetic operator on these variables to print the output {\tt `Hello World'}}
...
\ExeText{Comparison Operators output a {\tt True} or {\tt False} (Boolean) value.}
	...
	\Question{Write a program that compares the volume of a cuboid and the volume of a sphere (Exercise \ref{Ex:Numbers}) and outputs a message telling the user which is larger e.g.}
...
\begin{enumerate}[label=(\Alph*)]
    ...
    \item Write a program that sorts big cats into three categories using fur colour: Cats with brown fur are classified as lions, black fur are classified as pumas and black and brown fur as leopards.   
    ...
    \item Write a program, like the example shown in today's class, based on your own typical day, that tells you what action to do based on the time.   
