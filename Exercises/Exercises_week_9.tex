...
\begin{Exercise}[title= Histograms and importing data] \label{Ex:Variables}
	...
	\Question{Modify the {\tt diceRolls} function so it saves the results in a file diceRolls.csv, in the same folder as your code.\\
	\textbf{Note:} Remember to add {\tt import csv} at the start of your code.}

	\Question{Now import the dice rolls from your diceRolls.csv file and save it as a numpy array named {\tt data}. Print the values of {\tt data}.}
	\Question{Visualise that data. First, try using {\tt plt.plot()}. This shows the results of each dice roll, but is slightly confusing to look at. }
	\Question{Now, let's try to represent the distribution of dice rolls using a histogram. Use {\tt plt.hist()}  and {\tt plt.show()} to visualise the histogram. }
